...
2. **函数浏览**: 按调用关系排序，左右切换函数，上下滚动查看代码（语法高亮），自动标记已读，支持筛选未读函数，行级显示调用的项目内函数（f(N)按钮展开查看函数名+docstring，可点击跳转；支持多级属性调用如`self.strategy.method()`的解析，多候选时按调用表达式分组显示），后台自动预加载所有函数详情和AI解读（进度条显示）
